...
async function getPositionInfo() {
  try {
    ...
    // Debug: print raw bytes buat cari offset yang bener
    console.log("Account data length:", data.length);
    console.log("Bytes 8-72 (hex):", data.slice(8, 72).toString("hex"));

    // lb_pair: offset 8, 32 bytes
    ...
    // Coba beberapa offset buat lowerBinId & upperBinId
    // Struktur PositionV2: discriminator(8) + lbPair(32) + owner(32) + lowerBinId(4) + upperBinId(4)
    const offset = 8 + 32 + 32; // = 72
    const lowerBinId = data.readInt32LE(offset);
    const upperBinId = data.readInt32LE(offset + 4);
    console.log(`BinStep: ${binStep} | LowerBinId: ${lowerBinId} | UpperBinId: ${upperBinId}`);
