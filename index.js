...
function binIdToPrice(binId, binStep) {
  // Harga dalam lamport: (1 + binStep/10000)^binId
  // Adjust untuk decimal diff SOL(9) vs USDC(6)
  const rawPrice = Math.pow(1 + binStep / 10000, binId);
  return rawPrice * Math.pow(10, DECIMAL_DIFF);
}
...
async function getPositionInfo() {
  try {
    ...
    console.log(`BinStep: ${binStep} | Lower: ${lowerBinId} ($${lowerPrice.toFixed(2)}) | Upper: ${upperBinId} ($${upperPrice.toFixed(2)}) | Current: $${currentPrice.toFixed(2)}`);
